...
function loadProgress() {
    const saved = localStorage.getItem('stripe3Progress');
    if (saved) {
...
function saveProgress() {
    ...
    localStorage.setItem('stripe3Progress', JSON.stringify(data));
}
...
function submitQuiz() {
    ...
    if (passed) {
        ...
        // Clear progress
        localStorage.removeItem('stripe3Progress');
        
        // Show continue button
        setTimeout(() => {
            if (confirm('🎉 Stripe 3 Complete!\n\nContinue to Stripe 4?')) {
                window.location.href = 'white-belt-stripe4-interactive-FULL.html';
            } else {
