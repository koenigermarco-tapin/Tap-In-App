#!/usr/bin/env python3
"""
BATCH 1: Fix ARIA labels in PRODUCTION files only
...
def get_production_files_with_buttons_no_aria():
    ...
    # Check which ones have buttons without aria-label
    files_with_issues = []
    ...
    for filepath in prod_files:
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                content = f.read()
            buttons = re.findall(r'<button[^>]*>', content)
            buttons_no_aria = [b for b in buttons if 'aria-label' not in b]
            total += len(buttons_no_aria)
        except:
            pass
...
def main():
    print("=== BATCH 1: FIX PRODUCTION ARIA LABELS ===")
    ...
    files = get_production_files_with_buttons_no_aria()
