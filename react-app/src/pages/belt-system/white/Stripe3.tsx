...
const { lessons, quiz: quizQuestions } = whiteBeltContent[2]; // Stripe 3
...
export function WhiteBeltStripe3() {
  ...
  useEffect(() => {
    const savedLessons = localStorage.getItem('whiteBeltStripe3Lessons');
    const savedCheckpoints = localStorage.getItem('whiteBeltStripe3Checkpoints');
  ...
  useEffect(() => {
    localStorage.setItem('whiteBeltStripe3Lessons', JSON.stringify(completedLessons));
  }, [completedLessons]);

  useEffect(() => {
    localStorage.setItem('whiteBeltStripe3Checkpoints', JSON.stringify(completedCheckpoints));
  ...
  const handleCheckpointComplete = async (checkpointIndex: number) => {
    ...
    await addXP(25, `White Belt Stripe 3 - Checkpoint ${checkpointIndex + 1}`);
    confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 } });
    if (newCompleted.length === 4) {
      await addXP(100, 'White Belt Stripe 3 - Completion Bonus');
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <Card className="mb-8 bg-gradient-to-br from-slate-800 to-slate-900 border-slate-700">
          <div className="p-8">
            <div className="inline-flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full text-sm font-bold text-white mb-4">⚪ WHITE BELT - STRIPE 3 of 4</div>
            ...
            <h1 className="text-4xl font-bold text-white mb-4">🛠️ Building Team Trust</h1>
            <p className="text-slate-300 text-lg">Techniques to build trust systematically in your team.</p>
          </div>
        </Card>
              ...
              <Card className="mb-6 bg-slate-800 border-slate-700 overflow-hidden">
                <button onClick={() => toggleLesson(lesson.id)} className="w-full p-6 flex items-center justify-between hover:bg-slate-700/50 transition-colors">
                  <div className="flex items-center gap-4 flex-1 text-left">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold ${isCompleted ? 'bg-green-500' : 'bg-purple-500'} text-white`}>
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-slate-700 p-6 space-y-6">
                      ...
                      <Button onClick={() => completeLesson(lesson.id)} disabled={isCompleted} className={`w-full ${isCompleted ? 'bg-green-600' : 'bg-gradient-to-r from-purple-600 to-pink-600'}`}>{isCompleted ? <span className="flex items-center justify-center gap-2"><CheckCircle className="w-5 h-5" />Complete</span> : 'Mark Complete'}</Button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </Card>
        ...
        {allCheckpointsComplete && <StripeCompleteCard score={4} total={4} beltColor="white" stripeNumber={3} stripeName="Building Team Trust" onContinue={() => navigate('/belt-system/white/stripe-4')} />}
      </div>
    </div>
  );
}
