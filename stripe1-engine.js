...
function loadProgress() {
    const saved = localStorage.getItem('stripe1Progress');
    if (saved) {
...
function saveProgress() {
    ...
    localStorage.setItem('stripe1Progress', JSON.stringify(data));
}
...
function submitQuiz() {
    ...
    if (passed) {
        ...
        // Clear progress
        localStorage.removeItem('stripe1Progress');
        
        // Show continue button
        setTimeout(() => {
            if (confirm('🎉 Stripe 1 Complete!\n\nContinue to Stripe 2?')) {
                window.location.href = 'white-belt-stripe2-interactive-FULL.html';
            } else {
